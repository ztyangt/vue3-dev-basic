...
export async function listWatchlist(): Promise<{
  ...
  const items = await Promise.all(
    watchlist.map(async (item) => {
      ...
      const changePct =
        currentPrice && item.priceAtAdd ? parseFloat((((currentPrice - item.priceAtAdd) / item.priceAtAdd) * 100).toFixed(2)) : null;

      const toTargetPct =
        currentPrice && item.targetPrice ? parseFloat((((item.targetPrice - currentPrice) / currentPrice) * 100).toFixed(2)) : null;

      const toStopPct =
        currentPrice && item.stopPrice ? parseFloat((((item.stopPrice - currentPrice) / currentPrice) * 100).toFixed(2)) : null;
...
export async function checkAlerts(currentSignals?: Record<string, Verdict>): Promise<{ alerts: WatchlistAlert[]; count: number }> {
  const watchlist = await loadWatchlist();
