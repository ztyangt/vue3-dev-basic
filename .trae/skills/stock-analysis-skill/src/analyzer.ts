...
function buildDashboardPrompt(data: StockData, position: PositionInfo | undefined, warnings: string[]): string {
  const warningBlock = warnings.length > 0 ? `⚠️ 数据预警（必须在报告中体现）：\n${warnings.map((w) => `- ${w}`).join("\n")}\n\n` : "";
...
function buildReportPrompt(data: StockData, position: PositionInfo | undefined): string {
  const positionBlock = position?.status === "holding" ? `用户持仓成本：${position.cost ?? "未知"}` : "用户当前空仓";
...
export async function analyzeStock(
  ...
  const userPrompt = outputFormat === "markdown" ? buildDashboardPrompt(data, position, warnings) : buildReportPrompt(data, position);
...
export async function analyzeChartImage(imageUrlOrBase64: string, stockCode: string, isBase64 = false): Promise<string> {
  try {
    const zai = await ZAI.create();
